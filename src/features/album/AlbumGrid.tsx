...
const imagesGlob = import.meta.glob<{ default: string | { src: string } }>(
  "../../assets/album/*.{jpeg,jpg,png,gif,webp,JPG,JPEG,PNG,GIF,WEBP}",
  { eager: true },
);
...
const ALBUM_PHOTOS = Object.values(imagesGlob).map((img, idx) => {
  ...
  const src =
    typeof defaultExport === "string" ? defaultExport : defaultExport.src;
  ...
  return (
    <div className='grid mobile:grid-cols-1 tablet:grid-cols-3 desktop:grid-cols-4 gap-6 w-[60%] desktop:w-[80%] mt-6 mx-auto'>
      {ALBUM_PHOTOS.map((photo, index) => (
        <div
          key={index}
          className='overflow-hidden rounded-lg cursor-pointer relative group w-full aspect-3/4 shadow-md bg-dark-surface/50 border border-gray-800'>
          <Image
